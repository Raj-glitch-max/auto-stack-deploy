...
export function GitHubConnect() {
  ...
  const checkGitHubConnection = async () => {
    try {
      ...
      // Try to fetch user info to check for GitHub connection
      try {
        ...
        if (user.github_username) {
          ...
          // Try to fetch repos to verify connection
          try {
            await api.get("/github/repos")
            setIsConnected(true)
            // If we can fetch repos, GitHub is connected but username might not be set
    ...
    return (
      ...
      >
        ...
        <div className="flex flex-col min-w-0">
          ...
          {githubUsername && (
            <span className="text-xs text-gray-400 truncate">@{githubUsername}</span>
          )}
        </div>
